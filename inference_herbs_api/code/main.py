...
from data.dataset import build_loader,random_sample
from utils.costom_logger import timeLogger
from utils.config_utils import load_yaml, build_record_folder, get_args, update_record_yaml
...
def set_environment(args, tlogger):
    ...
    # device_ids : None --> use all gpus.
    model = torch.nn.DataParallel(model)
    model.to(args.device)
    if args.pretrained is not None:
        print('load pretrained successful')
...
def train(args, epoch, model, scaler, amp_context, optimizer, schedule, train_loader, logger):
    ...
    for batch_id, (ids, datas, labels) in enumerate(train_loader):
            ...
            for name in outs:
                ...
                if "FPN1_" in name:
                    if args.lambda_b0 != 0:
                        ...
                        thres = torch.Tensor(model.module.selector.thresholds[aux_name])
        ...
        if ((batch_id + 1) % args.log_freq == 0):
            ...
            cal_train_metrics(args, msg, outs, labels, batch_size, model.module.selector.thresholds)
...
def main(args, tlogger):
    ...
    log_path = os.path.join(args.save_dir, 'logger.log')
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if train_loader is not None:
            ...
            total_loss = train(args, epoch, model, scaler, amp_context, optimizer, schedule, train_loader, logger)
            tlogger.print()
        else:
        ...
        torch.save(checkpoint, os.path.join(args.save_dir, "backup", "last.pt"))
        torch.save(model_to_save, os.path.join(args.save_dir, 'model.pth'))
        ...
        if epoch == 0 or (epoch + 1) % args.eval_freq == 0:
            ...
            cfg['best_cls'] = best_eval_name
            with open("./configs/config.yaml", 'w') as file:
                yaml.dump(cfg, file, sort_keys=False)

            logger.info(f'[Evaluation] - best_epoch: {epoch + 1} best_eval_name: {best_eval_name} best_acc: {best_acc}')
            # for watchdog
            status = dict()
            status['epoch'] = epoch
            status['acc'] = best_acc
