...
def build_swintransformer(pretrained: bool = True,
    ...
    if conf['use_pretrained_backbone'] is True:
        # SKD 100 pretrained
        pretrained = torch.load('./pretrained/emb_func_best.pth')
        pretrained_dict = OrderedDict()
        modelsize = len(pretrained)
        count = 1
        for k,v in pretrained.items():
            if count < 5:
                pretrained_dict['patch_embed'+k[14:]] = v
            elif count > modelsize - 2 :
                pretrained_dict['norm'+k[14:]] = v
            else :
                pretrained_dict['layers'+k[14:]] = v
            count += 1
